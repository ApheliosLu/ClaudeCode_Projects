...
async def main():
    ...
    print("1) create_task: 后台任务不挡道:")  # → 1) create_task: 后台任务不挡道:
    task = asyncio.create_task(job("后台下载", 0.3))   # 提交即排队(不立刻跑)
    print("   已提交后台任务, 主流程继续…(任务稍后自己开始)")  # →    已提交后台任务, 主流程继续…(任务稍后自己开始)
    await asyncio.sleep(0.05)               # 主流程干点别的(后台任务趁机开跑)
    print("   主流程忙完别的, 现在 await 收结果:")  # →    主流程忙完别的, 现在 await 收结果:
    print("   →", await task)                # 等后台任务完成并取回返回值  →    → 后台下载 完成(耗时 0.3s)
    print("   (create_task 返回 Task —— Future 的 asyncio 版, 票据概念同 04/03)\n")
    # 输出:    (create_task 返回 Task —— Future 的 asyncio 版, 票据概念同 04/03)
    ...
    print("2) wait_for 限时(任务要 5s, 只等 0.3s):")  # → 2) wait_for 限时(任务要 5s, 只等 0.3s):

    async def slow_task():
        try:
            await asyncio.sleep(5)          # 模拟慢 API
        except asyncio.CancelledError:      # 被 wait_for 取消时收到这个
            print("   [slow_task] 收到取消信号, 清理后退出")  # →    [slow_task] 收到取消信号, 清理后退出
    ...
    try:
        ...
        print("   主流程捕获 TimeoutError → 等 0.3s 就放弃了, 不再干等 5s")  # →    主流程捕获 TimeoutError → 等 0.3s 就放弃了, 不再干等 5s
    print("   (asyncio.TimeoutError 在 3.11+ 与内置 TimeoutError 是同一个)\n")
    # 输出:    (asyncio.TimeoutError 在 3.11+ 与内置 TimeoutError 是同一个)
    ...
    print("3) to_thread: 阻塞调用不冻结循环:")  # → 3) to_thread: 阻塞调用不冻结循环:
    t0 = time.perf_counter()
    blocker = asyncio.to_thread(blocking_search, "py")  # 0.4s 阻塞活在后台线程
    fast = asyncio.create_task(job("快速小任务", 0.05))
    print("   [快速小任务]", await fast)      # 先回来 = 循环没被 blocker 冻住  →    [快速小任务] 快速小任务 完成(耗时 0.05s)
    print("   [blocker]", await blocker)     # 0.4s 后阻塞结果到达  →    [blocker] 阻塞搜索结果: py × N
    print(f"   总耗时 {time.perf_counter() - t0:.2f}s"
          " ≈ 0.4s + 线程启动开销 → 阻塞活没挡住别人")
          # 耗时随机器/负载浮动:
          #    总耗时 0.46s ≈ 0.4s + 线程启动开销 → 阻塞活没挡住别人
    print("   (关键证据是打印顺序: 快速任务在 blocker 之前就完成了 ——")  # →    (关键证据是打印顺序: 快速任务在 blocker 之前就完成了 ——
    print("    若循环被 0.4s 阻塞冻住, 它得干等到 0.4s 之后才有机会)\n")  # →     若循环被 0.4s 阻塞冻住, 它得干等到 0.4s 之后才有机会)

    # ---- 4. asyncio.Queue: 协程间的生产-消费 ----
    # 这是什么: asyncio.Queue —— 与 04/01 的 queue.Queue 同款 API(maxsize/put/
    #           get), 差别只在 put/get 要 await(满了/空了就挂起让位)。
    print("4) asyncio.Queue 生产者-消费者:")  # → 4) asyncio.Queue 生产者-消费者:
    ...
    async def producer():
        for i in range(1, 4):
            ...
            print(f"   生产者放入 {i}")
            # 生产者每放入一个输出一行(与消费者的行在终端交错: 放入1/处理1/放入2/...):
            #    生产者放入 1
            #    生产者放入 2
            #    生产者放入 3
    ...
    print("5) TaskGroup(3.11+): 一组任务, 一个失败全员撤退:")  # → 5) TaskGroup(3.11+): 一组任务, 一个失败全员撤退:

    async def good(name: str, sec: float):
        try:
            await asyncio.sleep(sec)
            print(f"   [good-{name}] 成功(睡了 {sec}s)")  # →    [good-A] 成功(睡了 0.1s)
        except asyncio.CancelledError:
            print(f"   [good-{name}] 被组取消(组里有人失败了)")  # →    [good-B] 被组取消(组里有人失败了)
    ...
    try:
        ...
        print(f"   主流程捕获 ExceptionGroup, 内含 {len(eg.exceptions)} 个异常:")  # →    主流程捕获 ExceptionGroup, 内含 1 个异常:
        for e in eg.exceptions:
            print(f"     - {type(e).__name__}: {e}")  # →      - ValueError: 数据库写入失败
    print("   (A 0.1s 已完成没被波及; B 睡 0.8s 未完成 → 被取消 → 全员撤退语义)\n")
    # 输出:    (A 0.1s 已完成没被波及; B 睡 0.8s 未完成 → 被取消 → 全员撤退语义)

    # ---- 6. 小结 ----
    print("6) 任务编排口诀:")  # → 6) 任务编排口诀:
    print("   * 一个任务慢慢等 → await; 多个任务一起 → gather")  # →    * 一个任务慢慢等 → await; 多个任务一起 → gather
    print("   * 后台任务不等它 → create_task; 限时等待 → wait_for")  # →    * 后台任务不等它 → create_task; 限时等待 → wait_for
    print("   * 同步阻塞/CPU 活 → to_thread(别裸调, 会冻循环)")  # →    * 同步阻塞/CPU 活 → to_thread(别裸调, 会冻循环)
    print("   * 协程间传数据 → asyncio.Queue; 同生共死 → TaskGroup")  # →    * 协程间传数据 → asyncio.Queue; 同生共死 → TaskGroup
