...
async def fetch(name: str, sec: float) -> str:
    ...
    # fetch 每次被调用时输出一行(本例 4 次: 先顺序 A/B, 后 gather 并发 A/B):
    #    [fetch-A] 发起请求, 等 0.5s…
    #    [fetch-B] 发起请求, 等 0.5s…
    #    [fetch-A] 发起请求, 等 0.5s…
    #    [fetch-B] 发起请求, 等 0.5s…
...
async def main():
    ...
    print("1) 协程是'可暂停函数', 调用不执行:")  # → 1) 协程是'可暂停函数', 调用不执行:

    async def hello():
        print("   (函数体执行了)")  # →    (函数体执行了)

    c = hello()
    print(f"   hello() 返回: {type(c).__name__} ← 注意上面没有'函数体执行了'")  # →    hello() 返回: coroutine ← 注意上面没有'函数体执行了'
    ...
    print("2) 顺序 await vs gather 并发(两个 0.5s 的'请求'):")  # → 2) 顺序 await vs gather 并发(两个 0.5s 的'请求'):
    ...
    print("4) 故意丢弃一个协程(看程序末尾 stderr 的 RuntimeWarning):")  # → 4) 故意丢弃一个协程(看程序末尾 stderr 的 RuntimeWarning):
    leftover = fetch("孤儿", 0.1)            # 不 await、不调度 → 永不执行
    # 程序结束时 stderr 出现(路径与行号随环境/版本):
    # D:\Miniforge3\envs\py314\Lib\asyncio\events.py:94: RuntimeWarning: coroutine 'fetch' was never awaited
    #   self._context.run(self._callback, *self._args)
    # RuntimeWarning: Enable tracemalloc to get the object allocation traceback
    print("   已创建但丢弃(函数体不会跑); 这是 bug 信号: 忘了 await 或 return\n")  # →    已创建但丢弃(函数体不会跑); 这是 bug 信号: 忘了 await 或 return
