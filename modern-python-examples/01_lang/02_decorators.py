...
print("1) 装饰器效果:")
print("   add(1, 2) =", add(1, 2), "  ← 装饰器没有改变 add(1,2) 的返回值")
...
print("\n2) functools.wraps:")
print("   greet.__name__ =", greet.__name__)         # 原始名 greet
print("   greet.__doc__  =", greet.__doc__)          # 原函数文档字符串保留
print("   greet('小明')  =", greet("小明"))           # 原函数照常工作, 外表与原名无异
...
print("\n3) 参数化装饰器 @retry(times=3):")
r = connect()                                        # 每次 80% 概率失败, 最多重试 3 次(概率事件, 多跑几次可能看到"连接成功!")
print(r if r else "（三次都失败时才走到这里, retry 的 wrapper 此时返回 None）")
...
print("\n4) 装饰器装饰类: 注册表中有:", sorted(registry.keys()))
...
class Demo:
    ...
    def limit(cls):
        print(f"5) classmethod 拿到类本身: {cls.__name__}.MAX = {cls.MAX}")

print()
Demo.limit()                                     # 无需实例, 直接类上调用
print("   staticmethod 同理:", Demo.add(3, 4))
