...
print("1) 当前解释器 GIL 状态:")  # → 1) 当前解释器 GIL 状态:
print("   sys._is_gil_enabled():", sys._is_gil_enabled())  # →    sys._is_gil_enabled(): True
print("   → True: 默认 GIL 构建(3.14 实测; 若 False 则是自由线程构建)")  # →    → True: 默认 GIL 构建(3.14 实测; 若 False 则是自由线程构建)
print("   Python 版本:", sys.version.split()[0])  # →    Python 版本: 3.14.3
...
try:
    ...
    print("\n2) 错误消息的拼写建议(3.13 增强的错误提示同族):")  # → 2) 错误消息的拼写建议(3.13 增强的错误提示同族):
    traceback.print_exc()
    # traceback 走 stderr(路径与行号随本文件位置/注释行数变化):
    # Traceback (most recent call last):
    #   File "<运行目录>\08_new\02_py313.py", line 35, in <module>
    #     print(unsplleable)                        # 故意拼错 → 触发 NameError
    #           ^^^^^^^^^^^
    # NameError: name 'unsplleable' is not defined. Did you mean: 'unspellable'?
    print("   ↑ traceback 尾行直接给出建议写法 —— 少一行猜谜时间")  # →    ↑ traceback 尾行直接给出建议写法 —— 少一行猜谜时间
...
print("\n3) TypeIs(3.13+) 与 TypeGuard(3.10+) 对照:")  # → 3) TypeIs(3.13+) 与 TypeGuard(3.10+) 对照:
for checker, tag in [(is_str_list, "TypeIs  "), (is_str_guard, "TypeGuard")]:
    print(f"   {tag}: 对 ['a', 'b'] → {checker(['a', 'b'])}; 对 [1, 2] → {checker([1, 2])}")
    # 两种检查函数各输出一行:
    #    TypeIs  : 对 ['a', 'b'] → True; 对 [1, 2] → False
    #    TypeGuard: 对 ['a', 'b'] → True; 对 [1, 2] → False
print("   两者运行时行为一致 —— 区别只在静态检查器能窄化多准(见注释)")  # →    两者运行时行为一致 —— 区别只在静态检查器能窄化多准(见注释)
print("   新代码用 TypeIs(3.13+); 要兼容旧版本才退到 TypeGuard")  # →    新代码用 TypeIs(3.13+); 要兼容旧版本才退到 TypeGuard
...
print("\n4) ReadOnly TypedDict 字段(3.13+):")  # → 4) ReadOnly TypedDict 字段(3.13+):
print("   构造实例:", m)  # →    构造实例: {'title': '星际穿越', 'year': 2014, 'rating': 8.7}
print("   读字段正常: rating =", m["rating"])  # →    读字段正常: rating = 8.7
print("   → 若代码里写 m['rating'] = 9.0, 静态检查器报错(运行时照常)")  # →    → 若代码里写 m['rating'] = 9.0, 静态检查器报错(运行时照常)
...
print("\n5) 背景名词(见注释, 无需代码):")  # → 5) 背景名词(见注释, 无需代码):
print("   dbm.sqlite3 后端(3.13 起默认) / 实验性 JIT(PEP 744) —— 普通用户零感知")
# 输出:    dbm.sqlite3 后端(3.13 起默认) / 实验性 JIT(PEP 744) —— 普通用户零感知
