...
def main():
    ...
    xs = [2_000_000] * 8
    print("1) map: CPU 活 8 个任务, 三种跑法计时:")
    ...
    print("3) map(提交序) vs as_completed(完成序):")
    delays = [0.4, 0.1, 0.3, 0.2]
    with cf.ThreadPoolExecutor(4) as ex:
        futs = [ex.submit(io_task, d) for d in delays]
        print("   提交顺序:   ", [f"任务{i}" for i in range(4)])
        print("   完成顺序:   ", end=" ")
    ...
    print("4) 异常传播(任务内 1 // 0):")
    with cf.ThreadPoolExecutor(2) as ex:
        f = ex.submit(boom, 0)
        time.sleep(0.1)
        print("   任务炸了, 但池还活着(别的任务照常) —— 异常在 result() 时才来:")
        try:
            f.result()
    ...
    print("   * 任务里的异常在 result() 时抛回主线程 → try/except 围在取结果处")
