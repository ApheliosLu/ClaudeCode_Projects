...
def main():
    ...
    print("0) 为什么需要多进程:")
    print("   01 结论: CPU 密集用线程 = GIL 一个线程在算, 其余干等(实测几乎零加速)")
    print("   → 这次直接开多个 Python 解释器(进程), 各算各的, 真并行")
    ...
    print("2) Pool.map: CPU 重活并行(8 个任务 × 400 万平方和):")
    xs = [4_000_000] * 8
    ...
    print(f"3) 共享内存 Value + Lock(两进程各 +{N:,} 次):")
    ...
    # ---- 5. 取舍总结 ----
    print("5) 取舍总结(什么时候用哪个):")
    print("   * CPU 密集(纯计算/训练/图像): 多进程 → 真并行; 代价: 启动贵、")
    print("     每进程一份解释器内存、任务太小(毫秒级)时并行反而更慢")
    print("   * IO 密集(网络/文件/爬虫): 线程或协程就够 → 别用进程(启动开销白花)")
    print("   * 进程间别惦记共享变量: 内存不互通, 通信走 Queue/Pipe")
