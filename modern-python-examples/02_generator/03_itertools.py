...
print("1) 无限迭代器 + islice:")  # → 1) 无限迭代器 + islice:
print("   list(islice(count(10, 5), 4)) =", list(islice(count(10, 5), 4)))
# 输出:    list(islice(count(10, 5), 4)) = [10, 15, 20, 25]
print("   list(islice(cycle('AB'), 6))  =", list(islice(cycle("AB"), 6)))
# 输出:    list(islice(cycle('AB'), 6))  = ['A', 'B', 'A', 'B', 'A', 'B']
print("   list(repeat(7, 3))            =", list(repeat(7, 3)))  # →    list(repeat(7, 3))            = [7, 7, 7]
# 这是什么: islice —— 迭代器的"切片": 与 list 切片语法类似, 但不建新列表, 直接截前 n 项。
print("   (repeat(x, n) = [x]*n 的惰性版, 拿完 n 个即止)")  # →    (repeat(x, n) = [x]*n 的惰性版, 拿完 n 个即止)
...
# 输出:    list(chain([1,2], (3,4), range(2))) = [1, 2, 3, 4, 0, 1]
...
print("   左支流取 2 个:", list(islice(a, 2)))  # →    左支流取 2 个: ['a', 'b']
print("   右支流取 2 个:", list(islice(b, 2)))  # →    右支流取 2 个: ['a', 'b']
print("   (两条流各自从头独立开始 —— 所以 tee 要趁早)")  # →    (两条流各自从头独立开始 —— 所以 tee 要趁早)
...
for name, group in groupby(fruits, key=lambda w: w[0]):
    ...
    #    banana: ['ripe', 'yellow']
    #    cherry: ['red']
print("   (忘了 sort 则 'apple' 会因不在相邻处而分成多组)")  # →    (忘了 sort 则 'apple' 会因不在相邻处而分成多组)
