...
print("1) 生成器基本形态:")
g = countdown(3)
print("   g 的类型:", type(g).__name__)          # generator, 不是函数执行结果
print("   next(g) →", next(g))
print("   next(g) →", next(g))                   # 看注释行: 现场被保留
print("   遍历取完:", list(countdown(3)))         # list() 把生成器全部跑完

# ---- 2. 生成器表达式 vs 列表推导: 惰性 + 内存 ----
print("\n2) 生成器表达式 vs 列表推导:")
gen = (x * x for x in range(1_000_000))          # 生成器表达式: () 而非 []
lst = [x * x for x in range(1_000_000)]          # 列表推导: 立刻全部算出
print(f"   生成器对象大小: {sys.getsizeof(gen):>6,} 字节(不存元素)")
print(f"   列表大小       : {sys.getsizeof(lst):>6,} 字节(100 万个整数)")
print("   生成器是按`要一个算一个`, 列表是`先全算好` → 大数据流选生成器")
print("   注意: 惰性 = 只走一遍; list(gen) 之后 gen 就空了")
...
print("\n3) send() 流式平均值:")
a = averaging()
print("   next 启动:", next(a))                      # None
print("   send(10):", a.send(10))                    # avg = 10
print("   send(20):", a.send(20))                    # avg = 15
print("   send(30):", a.send(30))                    # avg = 20
...
print("\n4) throw() / close():")
print("   ", next(r))
print("   ", r.throw(ValueError("炸弹")))            # 单参形式(新写法), 生成器内部接住
print("   ", next(r), " ← 继续还能正常工作")
...
def cleanup():
    try:
        ...
        print("   [generator.close() 触发 finally] 关闭资源: 连接池已清理")
...
def main_gen():
    ...
    print("   [yield from] 拿到子生成器 return:", result)
...
print("   (提示: 无限序列必须配合截取/条件再终止, 不能 list() 全接)")

# ---- 7. 生成器与列表的取舍一句话总结 ----
print("\n7) 取舍: 小数据/需要回头访问 → 列表; 大数据流/函数式管线 → 生成器")
print("   (生成器有惰性、省内存, 负作用: 不能 len()、不能下标、只能走一遍)")
