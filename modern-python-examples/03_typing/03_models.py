...
print("1) @dataclass:")  # → 1) @dataclass:
s = Student("小明", 95.5)
s2 = Student("小明", 95.5)
print("   自动 __repr__:", s)  # →    自动 __repr__: Student(name='小明', score=95.5, grade=1)
print("   自动 __eq__:", s == s2)  # →    自动 __eq__: True
print("   自动 __init__ 与属性访问:", s.name, s.score, f"年级{s.grade}")  # →    自动 __init__ 与属性访问: 小明 95.5 年级1
...
try:
    ...
    # 输出:    [dataclass 时代] tags: list = [] → 类定义时就 ValueError:
    print("     ", e)  # →       mutable default <class 'list'> for field tags is not allowed: use default_factory
...
oa.tags.append("数学")
print("   [普通类时代] def __init__(self, tags=[]): 是共享的 ——")  # →    [普通类时代] def __init__(self, tags=[]): 是共享的 ——
print("     oa.tags =", oa.tags, "; ob.tags =", ob.tags, "← oa 改了 ob 也跟着! (这才是活着的坑)")
# 输出:      oa.tags = ['数学'] ; ob.tags = ['数学'] ← oa 改了 ob 也跟着! (这才是活着的坑)
...
# 输出:    [正解 default_factory] bk1.tags = ['计算机'] ; bk2.tags = [] ← 各自独立
...
p = Point(1, 2.5)
print("\n3) frozen=True:")  # → 3) frozen=True:
print("   Point(1, 2.5) =", p)  # →    Point(1, 2.5) = Point(x=1, y=2.5)
try:
    p.x = 9
except FrozenInstanceError as e:
    print("   改 frozen 字段报错:", e)  # →    改 frozen 字段报错: cannot assign to field 'x'
...
ce = CacheEntry("a", 3)
print("\n4) @dataclass(slots=True)(3.10+):")  # → 4) @dataclass(slots=True)(3.10+):
print("   CacheEntry =", ce, " ← 与普通 dataclass 用法相同, 但实例更省内存")
# 输出:    CacheEntry = CacheEntry(key='a', hits=3)  ← 与普通 dataclass 用法相同, 但实例更省内存
...
print("   Color(255, 0, 128): 字段访问", c.r, c.g, c.b,
      ...
      # 输出:    Color(255, 0, 128): 字段访问 255 0 128 ; 索引 c[0] 255 ; 解包 255 0 128
print("   元组身份:", isinstance(c, tuple), " ← 它就是元组, 可进任何元组上下文")  # →    元组身份: True  ← 它就是元组, 可进任何元组上下文
...
print("\n6) TypedDict:")  # → 6) TypedDict:
print("   m 运行时就是普通 dict:", m, type(m).__name__)  # →    m 运行时就是普通 dict: {'title': '星际穿越', 'year': 2014} dict
print("   (检查器视角: m['titrle'] 拼写错误会被揪出 —— dict 则抓不着)")  # →    (检查器视角: m['titrle'] 拼写错误会被揪出 —— dict 则抓不着)
...
print("\n7) Enum / StrEnum(3.11+) / IntEnum:")  # → 7) Enum / StrEnum(3.11+) / IntEnum:
print("   Direction.NORTH:", Direction.NORTH, "| .name =", Direction.NORTH.name, "| .value =", Direction.NORTH.value)
# 输出:    Direction.NORTH: Direction.NORTH | .name = NORTH | .value = 1
print("   遍历全部成员:", [d.name for d in Direction])  # →    遍历全部成员: ['NORTH', 'SOUTH', 'EAST', 'WEST']
by_value = {d.value: d for d in Direction}          # 值反查成员的惯用姿势
print("   值反查: by_value[2] =", by_value[2].name)  # →    值反查: by_value[2] = SOUTH
print("   StrEnum 与字符串互通: Colors.RED == 'red' →", Colors.RED == "red",
      "; Colors.RED.upper() =", Colors.RED.upper())
      # 输出:    StrEnum 与字符串互通: Colors.RED == 'red' → True ; Colors.RED.upper() = RED
print("   IntEnum 与整数互通: Status.OK == 200 →", Status.OK == 200)  # →    IntEnum 与整数互通: Status.OK == 200 → True
