...
print("\n3) 自定义对象序列化(default 钩子):")
print("   dumps 结果:", s_json)
...
print("\n4) pickle(仅演示机制, 红线在注释):")
data = {"cache": [1, 2, 3], "note": "自己程序内部缓存"}
blob = pickle.dumps(data)
print("   dumps 字节:", blob[:28], "...   # 二进制, 人类不可读")
print("   loads 还原:", pickle.loads(blob))
print("   ⚠️ 只反序列化自己信任来源的数据 —— 对不可信输入用 pickle 等同裸奔")
...
print("   c 的 list 加 4 后:")
print("   a['items']:", a["items"], "  # 跟着变了(共享内层)")
print("   d['items']:", d["items"], "  # 不受影响(深拷贝独立)")
d["items"].append(5)
print("   d 再加 5 → a 仍是:", a["items"], "  # 彻底互不干扰")
...
#           此处不展开(纯标准库, 需要时查文档即会)。
print("\n6) shelve: 迷你持久化 dict(注释已述, 不展开演示)")
print("\n全部演示完成 ✔")
