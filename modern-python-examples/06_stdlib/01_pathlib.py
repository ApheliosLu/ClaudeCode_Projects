...
print("1) 路径对象与 os.path 对照表:")  # → 1) 路径对象与 os.path 对照表:
p = Path("docs") / "report" / "2026.md"
print("   拼接结果      :", p, "   # Windows 下自动用反斜杠显示")  # →    拼接结果      : docs\report\2026.md    # Windows 下自动用反斜杠显示
print("   os.path 旧写法: os.path.join('docs', 'report', '2026.md')")
# 输出:    os.path 旧写法: os.path.join('docs', 'report', '2026.md')
print("   文件后缀 suffix:", p.suffix, "    # .md; 等价旧写法 os.path.splitext()[1]")
# 输出:    文件后缀 suffix: .md     # .md; 等价旧写法 os.path.splitext()[1]
print("   文件名 name    :", p.name)          # 最后一段: 2026.md  →    文件名 name    : 2026.md
print("   主名 stem      :", p.stem)          # 去掉后缀: 2026  →    主名 stem      : 2026
print("   父目录 parent  :", p.parent)  # →    父目录 parent  : docs\report

# ---- 2. cwd / home / 绝对化 ----
print("\n2) 常用起点路径:")  # → 2) 常用起点路径:
print("   当前工作目录  :", Path.cwd())
# 绝对路径随运行位置变化:
#    当前工作目录  : <运行目录>\06_stdlib
print("   用户主目录    :", Path.home())
# 随机器/用户而定:
#    用户主目录    : C:\Users\<用户名>
print("   相对转绝对    :", p.resolve())
# 随运行位置变化:
#    相对转绝对    : <运行目录>\docs\report\2026.md

# ---- 3. 实战: 临时目录里建一棵树, 再遍历它 ----
# 这是什么: tempfile.TemporaryDirectory —— 用完自动删除的临时目录
#           (上下文管理器, 退出 with 即清理; 免去手动 rm -rf 的残留烦恼)。
print("\n3) 在临时目录里建目录树并用 Path 遍历:")  # → 3) 在临时目录里建目录树并用 Path 遍历:
with tempfile.TemporaryDirectory(prefix="pathlib_demo_") as tmp:
    ...
    print("   --- glob('*.py') 只找本层 .py ---")  # →    --- glob('*.py') 只找本层 .py ---
    for f in sorted(root.glob("*.py")):
        print("   ", f.name)                        # src 下的 main.py 不会被 glob 到
        # 根目录这一层没有 .py 文件, 循环体一次都不执行(故本行没有输出):
        #    
    print("   --- rglob('*.py') 递归找全部 .py ---")  # →    --- rglob('*.py') 递归找全部 .py ---
    ...
    print("   --- Path.walk() (3.12+) ---")  # →    --- Path.walk() (3.12+) ---
    for dirpath, dirnames, filenames in root.walk():
        ...
        #    目录 .: ['README.md']
        #    目录 src: ['main.py']
        #    目录 src\utils: ['helper.py']
    # 这是什么: Path.walk() —— 3.12 新增, 递归遍历生成 (目录, 子目录名列表, 文件名列表)
    #           三元组, 逐个目录往下走; (3.12 及之前: 用 os.walk(root) 得到的是
    #           字符串路径, 还要自己再包 Path)。

    print("   --- stat / is_file / exists ---")  # →    --- stat / is_file / exists ---
    main = root / "src" / "main.py"
    print("   is_file():", main.is_file(), "  exists():", main.exists())  # →    is_file(): True   exists(): True
    print("   文件大小    :", main.stat().st_size, "字节")  # →    文件大小    : 13 字节
...
print("\n4) 本脚本 __file__ 定位自身:")  # → 4) 本脚本 __file__ 定位自身:
print("   绝对路径  :", here)
# 随运行位置变化:
#    绝对路径  : <运行目录>\01_pathlib.py
print("   文件名    :", here.name, "   # 带 .py 后缀 —— 演示点")  # →    文件名    : 01_pathlib.py    # 带 .py 后缀 —— 演示点
