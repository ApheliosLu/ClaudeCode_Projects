...
工作流 skill **不在本项目内**，统一装在全局 `~/.claude/skills/`，所有项目共享。

| 来源 | 数量 | 更新方式 |
| --- | --- | --- |
| superpowers-zh 框架 | 20 | `npx superpowers-zh` |
| 社区手动安装（`grill-with-docs`、`improve-codebase-architecture`） | 2 | 无自动更新渠道，换机器需手动移植 |

**本项目不再自带 skills 副本。** 历史上曾放在 `.claude/skills/`，2026-09-22 移除——
原因是它们属于跨项目的通用工具而非项目依赖，留在仓库里会造成版本分叉与多项目重复。
需要找回当时的内容见 git 历史。

skill 的加载时机与使用方式见全局 `~/.claude/CLAUDE.md` 的「工作方式」一节。
